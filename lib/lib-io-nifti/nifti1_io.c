...
nifti_image * nifti_copy_nim_info(const nifti_image * src)
{
  ...
  if( src->fname ) dest->fname = strdup(src->fname);
  if( src->iname ) dest->iname = strdup(src->iname);
  dest->num_ext = 0;
  dest->ext_list = NULL;
...
static int rci_alloc_mem(void ** data, int prods[8], int nprods, int nbyper )
{
   ...
   for( idx = 0, size = 1; idx < nprods; idx++ )
       size *= prods[idx];
                                              ...
                                              int prods[], int * nprods )
{
   ...
   while( idx > 0 ){
      prods[len] = 1;
      while( idx > 0 && (nim->dim[idx] == 1 || dims[idx] == -1) ){
         prods[len] *= nim->dim[idx];
         idx--;
      }
      pivots[len] = idx;
      len++;
      idx--;  /* fine, let it drop out at -1 */
   }
   ...
   *nprods = len;
   ...
   if( g_opts.debug > 2 ){
      ...
      for(idx = 0; idx < len; idx++) fprintf(stderr," %d", pivots[idx]);
      fprintf(stderr,", prods :");
      for(idx = 0; idx < len; idx++) fprintf(stderr," %d", prods[idx]);
      fputc('\n',stderr);
   }
